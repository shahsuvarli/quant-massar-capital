...
def optimize_portfolio(request):
    portfolio = Portfolio.objects.get(id=request.query_params.get('portfolio_id'))
    ...
    cov_matrix = np.random.rand(len(tickers), len(tickers))  # Simulating covariance matrix
    ...
    result = sco.minimize(portfolio_volatility, num_assets * [1./num_assets,], method='SLSQP', bounds=bounds, constraints=constraints)
